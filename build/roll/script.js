...
socket.on("connect", () => {
  socket.emit("roll", {
    init: { id: getId(), hash: localStorage.getItem("hash") },
  });
});
...
function clicked() {
  ...
  socket.emit(
    ...
    {
      id: id || getId(),
      stakes: parseInt(document.getElementById("stake").innerHTML || "1000"),
      hash: localStorage.getItem("hash"),
    },
    (data) => startRolling(data),
  );
...
async function autoroll() {
  if (autorolling) {
    autorolling = false;
    autorollbtn.innerHTML = "autoroll";
    document.getElementById("startbtn").disabled = false;
    return;
  }

  autorolling = true;
  autorollbtn.innerHTML = "autorolling..";
...
async function startAutoRolling(result) {
  if (result) {
    if (result.notenough) {
      new bootstrap.Toast(document.getElementById("notenoughtoast")).show();

      document.getElementById("startbtn").innerHTML = "roll";
      document.getElementById("startbtn").disabled = false;
      autorollbtn.innerHTML = "autoroll";
      autorollbtn.disabled = false;

      return (autorolling = false);
    }
    if (result.noacc) {
  ...
  const finalLeftPosition =
    (totalWidth - outerslider.offsetWidth / 2 - Math.random() * itemWidth) * -1;
